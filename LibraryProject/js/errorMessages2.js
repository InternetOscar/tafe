...
function addErrorMessage(x, id, msg) {
   	'use strict';
    ...
    var newId = 'x' + id + 'Error';
    
    // Check for the existence of the span:
    var span = document.getElementById(newId);
    if (span) {
        span.parentNode.value = x;
        span.nextSibling.value = msg; // Update
    } else { // Create new.
        ...
        span.insertBefore(document.createTextNode(x));
...
function removeErrorMessage(id) {
    ...
    var span = document.getElementById('x' + id + 'Error');
	if (span) {
